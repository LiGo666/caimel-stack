...
import { getTranslations as getTranslationsGeneric } from "next-intl/server"
...
export async function testIntlAction(): Promise<ApiResponse<ActionResult>> {
   ...
   try {
      ...
      if (!rateLimitResult.success) return rateLimitResult as ApiResponse<ActionResult>
      ...
      if (isSuccess) {
         ...
         return { success: false, toastTitle: t("requests.notfound.title"), toastDescription: t("requests.notfound.description"), toastType: "error" }
      }
   } catch (error) {
