...
import { validatePassphrase, getPathByPath } from "@/features/secureApi/lib/managePathPassphrases"
...
export async function getNumbers(passphrase?: string) {
   try {
      ...
      try {
         ...
         const isValid = await validatePassphrase({ path: currentPath, passphrase: passphrase })
         ...
         console.error("Database error while validating path/passphrase:", dbError)
         return { authenticated: false, numbers: [], error: "Error validating credentials", path: currentPath }
      }
   } catch (error) {
      console.error("Unexpected error in getNumbers:", error)
      return { authenticated: false, numbers: [], error: "An unexpected error occurred" }
   }
}
