...
export const createPathPassphraseSchema = pathSchema.extend({
  passphrases: z
    .array(
      z.string().min(8, { message: "Passphrase must be at least 8 characters" })
    )
...
export const getPathByIdSchema = z.object({
  id: z.cuid({ message: "Invalid path ID" }),
});
...
export const deletePathSchema = z.object({
  id: z.string().cuid({ message: "Invalid path ID" }),
});

// Schema for validating a passphrase for a path
export const validatePassphraseSchema = pathSchema.extend({
  passphrase: z.string().min(1, { message: "Passphrase is required" }),
});

// Types derived from schemas
export type CreatePathPassphraseInput = z.infer<
  typeof createPathPassphraseSchema
>;
export type UpdatePathPassphraseInput = z.infer<
  typeof updatePathPassphraseSchema
>;
