"use server"
import { z } from "zod"
import { createPath, getPathById, getPathByPath, updatePath, deletePath, validatePassphrase, getAllPaths } from "../lib/managePathPassphrases"
import {
   ...
   try {
      ...
      if (error instanceof z.ZodError) {
         return {
            ...
            toastDescription: error.issues.map((issue) => `${issue.path.join(".")}: ${issue.message}`).join("; "),
            toastType: "error",
         }
      }
...
export async function getPathByIdAction(data: unknown): Promise<ApiResponse> {
   try {
      ...
      if (!result) {
         return { success: false, toastTitle: t("pathNotFound"), toastType: "error" }
      }
      return { success: true, data: result, toastTitle: t("pathRetrieved"), toastType: "success" }
   } catch (error) {
      const t = await getTranslations("features.secure-api")
      if (error instanceof z.ZodError) {
         return {
...
export async function getPathByPathAction(data: unknown): Promise<ApiResponse> {
   try {
      ...
      if (!result) {
         return { success: false, toastTitle: t("pathNotFound"), toastDescription: t("pathNotFound"), toastType: "error" }
      }
      return { success: true, data: result }
   } catch (error) {
      const t = await getTranslations("features.secure-api")
      if (error instanceof z.ZodError) {
         return { success: false, toastTitle: t("validationFailed"), toastDescription: t("validationFailed"), toastType: "error" }
      }
      return unexpectedErrorToastContent(t, "ERROR-123456")
   }
}
 ...
 */
export async function validatePassphraseAction(data: unknown): Promise<ApiResponse> {
   try {
      const t = await getTranslations("features.secure-api")
