...
import { rateLimitProfiles, RateLimitScenarioType } from "../config/rateLimitProfiles"
...
async function getContextInfos() {
   ...
   const ip =
      headersList.get("cf-connecting-ip") || headersList.get("x-forwarded-for")?.split(",")[0]?.trim() || headersList.get("x-real-ip") || "unknown_ip"
   const country = headersList.get("cf-ipcountry") || "unknown_country"
   const method = headersList.get("x-invoke-method") || headersList.get("method") || "unknown_method"
   const path = headersList.get("x-invoke-path") || headersList.get("next-url") || "unknown_path"
   ...
   return { userId, userAgent, referrer, ip, country, method, path, apiKey, rateLimitKey }
}

export async function assertRatelimit<T extends Record<string, any> = {}>(scenarioType: RateLimitScenarioType): Promise<ApiResponse<T>> {
   if (!redis) {
      throw new Error("Redis client not initialized. Ensure Redis feature is configured.")
   }
