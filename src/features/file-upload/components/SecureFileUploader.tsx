...
import { useSecureUploadStrategy, UploadProgress } from "../hooks/useSecureUploadStrategy"
...
export function SecureFileUploader({
  ...
  const { uploadFile, abortUpload, abortAllUploads } = useSecureUploadStrategy()
  ...
  // Validate file before adding - note we don't need client-side validation
  // since our secure server action will handle all validation
  const validateFile = useCallback((file: File): string | null => {
    // We only do minimal client-side validation for UX purposes
    // The real validation happens on the server
    
    // Basic size check to prevent obviously too large files
    if (file.size > 100 * 1024 * 1024) { // 100MB
      return `File size exceeds the maximum allowed size of 100MB`
    }
        ...
        semaphore.map(async () => {
          while (fileIndex < pendingFiles.length) {
            ...
            try {
              ...
              // Note: we don't pass config here as it's defined server-side
              const result = await uploadFile(fileState.file, undefined, {
                onProgress: (progress) => {
                  setFileStates(prev => ({
                    ...prev,
              ...
              console.log(`[SECURE_UPLOADER] Successfully uploaded: ${result.name}`)

            } catch (error) {
              const errorMessage = error instanceof Error ? error.message : 'Unknown upload error'
              console.error(`[SECURE_UPLOADER] Upload failed for ${fileState.file.name}:`, error)
  ...
  }, [pendingFiles, onUploadStart, onUploadComplete, onUploadError, uploadFile, fileStates])
    ...
    <div className={`flex flex-col gap-4 ${className}`}>
      ...
      >
        ...
        <p className="text-xs text-gray-400 mt-1">
          Secure upload - validation happens server-side
        </p>
        <input
          ref={fileInputRef}
          type="file"
