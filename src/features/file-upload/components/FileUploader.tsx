...
import { useUploadStrategy, UploadProgress } from "../hooks/useUploadStrategy"
...
export function FileUploader({
  ...
  const { uploadFile, abortUpload, abortAllUploads } = useUploadStrategy()
        ...
        semaphore.map(async () => {
          while (fileIndex < pendingFiles.length) {
            ...
            try {
              ...
              console.log(`[UNIFIED_UPLOADER] Successfully uploaded: ${result.name}`)

            } catch (error) {
              const errorMessage = error instanceof Error ? error.message : 'Unknown upload error'
              console.error(`[UNIFIED_UPLOADER] Upload failed for ${fileState.file.name}:`, error)
  ...
  // Get upload method indicator
  const getUploadMethod = (fileSize: number): string => {
    return fileSize > 50 * 1024 * 1024 ? "Chunked" : "Direct"
  }
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        {showProgressDetails && (
                          <span className="text-xs bg-gray-100 px-2 py-1 rounded">
                            {getUploadMethod(fileState.file.size)}
                          </span>
                        )}
                      </div>
