      ...
      const interval = setInterval(async () => {
         try {
            ...
            const mockJobs: JobProgress[] = [
               ...
               { jobId: "2", type: "DIARIZATION", progress: 0, status: "QUEUED", message: "Waiting for transcription..." },
            ]
      ...
      <Card className="w-full">
         <CardHeader>
            <CardTitle className="flex items-center gap-2">
               {isComplete ? <CheckCircle className="h-5 w-5 text-green-500" /> : <Loader2 className="h-5 w-5 animate-spin text-blue-500" />}
               {t("title")}
            </CardTitle>
         </CardHeader>
               ...
               <div key={job.jobId} className="space-y-2">
                  <div className="flex items-center justify-between">
                     <div className="flex items-center gap-2">
                        ...
                        <Badge className={getStatusColor(job.status)}>{t(`status.${job.status.toLowerCase()}`)}</Badge>
                     </div>
