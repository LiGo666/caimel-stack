...
export async function generateFileUploadUrl(
  ...
  try {
    ...
    if (validatedRequest.fileSize > config.maxFileSize) {
      return {
        success: false,
        error: `File size exceeds the maximum allowed size of ${config.maxFileSize / (1024 * 1024)}MB`,
      };
    }
    ...
    if (bucketExists) {
      ...
      try {
        ...
        if (notificationsExist) {
          console.log(
            `Bucket ${config.bucketName} exists and has notifications set up`
          );
        } else {
          console.log(
            `Bucket ${config.bucketName} exists but has no notifications, setting them up...`
          );
          await setupBucketNotifications(minioClient, config);
        }
      } catch (error) {
        console.error(
          `Error checking bucket notifications for ${config.bucketName}:`,
          error
        );
      ...
      try {
        console.log(
          `Bucket ${config.bucketName} does not exist, creating it...`
        );
        ...
        return {
          success: false,
          error: `Error creating bucket ${config.bucketName}: ${error}`,
        };
      }
    }

    // Helper function to set up bucket notifications
    async function setupBucketNotifications(
      client: MinioClient,
      ...
      try {
        ...
        console.log(
          `Webhook notification for bucket ${cfg.bucketName} set up successfully`
        );
      } catch (error) {
        console.error(
          `Error setting up webhook notification for bucket ${cfg.bucketName}:`,
          error
        );
        throw error;
      }
    ...
    console.log(
      `Created upload session in database with ID: ${uploadSession.id} for object: ${objectName}`
    );
