...
const WATCHPACK_POLLING = z.coerce
  .boolean()
  .optional()
  .default(false)
  .parse(process.env.WATCHPACK_POLLING);
const WATCHPACK_POLLING_INTERVAL = z.coerce
  .number()
  .int()
  .positive()
  .optional()
  .default(200)
  .parse(process.env.WATCHPACK_POLLING_INTERVAL);
const NEXT_TELEMETRY_DISABLED = z.coerce
  .boolean()
  .optional()
  .default(true)
  .parse(process.env.NEXT_TELEMETRY_DISABLED);
const PNPM_STORE_DIR = z.string().optional().parse(process.env.PNPM_STORE_DIR);

const ADMIN_USERNAME = z
  .string()
  .min(1, "ADMIN_USERNAME is required")
  .parse(process.env.ADMIN_USERNAME);
...
const NEXTAUTH_URL = z
  ...
  .parse(process.env.NEXTAUTH_URL);
const NEXTAUTH_SECRET = z
  .string()
  .min(1, "NEXTAUTH_SECRET is required")
  .parse(process.env.NEXTAUTH_SECRET);
const NEXTAUTH_TRUST_HOST = z.coerce
  .boolean()
  .optional()
  .default(true)
  .parse(process.env.NEXTAUTH_TRUST_HOST);

const REDIS_HOSTNAME = z
  .string()
  .min(1, "REDIS_HOSTNAME is required")
  .parse(process.env.REDIS_HOSTNAME);
const REDIS_PASSWORD = z
  .string()
  .min(1, "REDIS_PASSWORD is required")
  .parse(process.env.REDIS_PASSWORD);

// MinIO configuration
const MINIO_HOST = z
  .string()
  .min(1, "MINIO_HOST is required")
  .parse(process.env.MINIO_HOST);
const MINIO_ACCESS_KEY = z
  .string()
  .min(1, "MINIO_ACCESS_KEY is required")
  .parse(process.env.MINIO_ACCESS_KEY);
const MINIO_SECRET_KEY = z
  .string()
  .min(1, "MINIO_SECRET_KEY is required")
  .parse(process.env.MINIO_SECRET_KEY);
...
const MINIO_NOTIFY_WEBHOOK_ENDPOINT = z
  .string()
  .min(1, "MINIO_NOTIFY_WEBHOOK_ENDPOINT is required")
  .parse(process.env.MINIO_NOTIFY_WEBHOOK_ENDPOINT);
...
const SECUREAPI_FAIL_MODE = z
  .enum(["block", "allow"])
  .optional()
  .default("block")
  .parse(process.env.SECUREAPI_FAIL_MODE);
const SECUREAPI_TURNSTILE_TIMEOUT_MS = z.coerce
  .number()
  .int()
  .positive()
  .optional()
  .default(3000)
  .parse(process.env.SECUREAPI_TURNSTILE_TIMEOUT_MS);
const SECUREAPI_TRUST_CLOUDFLARE = z.coerce
  .boolean()
  .optional()
  .default(true)
  .parse(process.env.SECUREAPI_TRUST_CLOUDFLARE);
// Optional shared secret header for origin hardening (Cloudflare Transform Rule recommended)
const SECUREAPI_ORIGIN_SHARED_SECRET = z
  .string()
  .optional()
  .parse(process.env.SECUREAPI_ORIGIN_SHARED_SECRET);
// RL defaults (can be tuned per-route)
const SECUREAPI_RL_POINTS = z.coerce
  .number()
  .int()
  .positive()
  .optional()
  .default(100)
  .parse(process.env.SECUREAPI_RL_POINTS);
const SECUREAPI_RL_WINDOW_SECONDS = z.coerce
  .number()
  .int()
  .positive()
  .optional()
  .default(60)
  .parse(process.env.SECUREAPI_RL_WINDOW_SECONDS);
const SECUREAPI_RL_BLOCK_SECONDS = z.coerce
  .number()
  .int()
  .nonnegative()
  .optional()
  .default(60)
  .parse(process.env.SECUREAPI_RL_BLOCK_SECONDS);
...
const POSTGRES_DATABASE_URL = z
  ...
  .refine(
    ...
    "POSTGRES_DATABASE_URL must start with 'postgres://'"
  )
