import { VoiceTrainingConfig } from "../types"
...
export class VoiceTrainer {
   ...
   static async validateTrainingData(
      ...
      return { valid: issues.length === 0, totalDuration, issues }
   }

   static async getRecommendedHyperparameters(modelType: string, datasetSize: number): Promise<Record<string, any>> {
      switch (modelType) {
         case "XTTS_V2":
            return {
         ...
         default:
            return { epochs: 500, learningRate: 0.0001, batchSize: 4, gradientClipping: 1.0 }
      }
   }
}
