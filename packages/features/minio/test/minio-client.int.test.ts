...
describe("MinioClient Integration Tests", () => {
  ...
  beforeAll(() => {
    ...
    fs.writeFileSync(
      tempFilePath,
      "This is test content for Minio integration test"
    );
  ...
  describe("Bucket and notification lifecycle", () => {
    ...
    it("should check that bucket notifications don't exist initially", async () => {
      const notificationsExist =
        await minioClient.bucketNotificationExists(testBucketName);
    ...
    it("should generate a presigned URL and successfully upload a file", async () => {
      ...
      try {
        const response = await axios.post(presignedResult.url, formData, {
